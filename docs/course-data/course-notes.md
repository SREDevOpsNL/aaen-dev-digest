...
2. **Check regular login to the LMS**
   ...
   - Try logging in with `<your_login>`.
...
**Important note:** in the text supplied, the raw URLs themselves are not visible — only the words "link" and "Login" appear. In the original email they are presumably clickable. Also note: BetterED registered the account under `<your_login>`. Use that address, not a different Gmail address. So the first action is: open the LMS registration link → create a password → log in → then join the learning space. After that, you can start on the base materials.
